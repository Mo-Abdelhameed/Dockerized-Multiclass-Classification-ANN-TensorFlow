...
def train_predictor_model(
        train_inputs: pd.DataFrame, train_targets: pd.Series, hyperparameters: dict, output_dim: int
) -> Classifier:
    """
    ...
    """
    classifier = Classifier(output_dim, **hyperparameters)
    classifier.fit(train_inputs=train_inputs, train_targets=train_targets)
    return classifier


def predict_with_model(
        classifier: Classifier, data: pd.DataFrame
) -> np.ndarray:
    """
    Predict class probabilities for the given data.
    ...
    """
    return classifier.model.predict(data)
